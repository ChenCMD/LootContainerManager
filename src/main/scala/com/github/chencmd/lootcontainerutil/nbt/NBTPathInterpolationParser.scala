...
object NBTPathInterpolationParser extends RegexParsers {
  ...
  private def nextTextPart: Parser[String] = untilNextDelimiter | untilTheEnd

  private def untilTheEnd: Parser[String] = ".*$".r.map(_.replace("\\%", "%"))

  // % を一個残してパースする
  private def untilNextDelimiter: Parser[String] = ".*?(?<!\\\\)%(?=%)".r.map(_.dropRight(1).replace("\\%", "%"))
