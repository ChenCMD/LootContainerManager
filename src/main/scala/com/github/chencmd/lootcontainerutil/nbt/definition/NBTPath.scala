...
case class NBTPath(root: NBTPathRootNode, nodes: List[NBTPathNode]) {
  ...
  def access(tag: NBTTagCompound): List[NBTTag] = {
    def advanceFromRoot(node: NBTPathRootNode)       = StateT.modifyF[Option, NBTTag] { tag =>
      import NBTPathRootNode.*
      node match {
        case MatchRootObject(pattern)   => Option.when(isMatch(tag, pattern))(tag)
        case MatchObject(name, pattern) => getChild(tag, name).flatMap { child =>
            Option.when(isMatch(child, pattern))(child)
          }
        case CompoundChild(name)        => getChild(tag, name)
      }
    }
    def advanceIntoChildrenNondet(node: NBTPathNode) = StateT.modifyF[List, NBTTag] { tag =>
      import NBTPathNode.*
      node match {
        case NBTPathNode.MatchObject(name, pattern) => getChild(tag, name).flatMap { child =>
            Option.when(isMatch(child, pattern))(child)
          }.toList
        case NBTPathNode.AllElements()              => tag
            .downcastOrNone[NBTTagList]
            .fold(List.empty)(_.toList)
        case NBTPathNode.MatchElement(pattern)      => tag
            .downcastOrNone[NBTTagList]
            .fold(List.empty)(_.toList)
            .filter {
            ...
            .toList
        case NBTPathNode.IndexedElement(index)      => tag
            .downcastOrNone[NBTTagList]
            .flatMap(_.toList.pipe { list =>
              list.lift(Math.floorMod(index, list.length))
            })
            .toList
        case NBTPathNode.CompoundChild(name)        => getChild(tag, name).toList
      }
    }

    val optionToList = FunctionK.lift[Option, List]([A] => (_: Option[A]).toList)
    val state        = advanceFromRoot(root)
  ...
  private def isMatch(target: NBTTag, expect: NBTTag): Boolean = {
    (target, expect) match {
      case (NBTTagCompound(tMap), NBTTagCompound(eMap))     => eMap.forall {
          case (key, eValue) => tMap.get(key).exists { tValue =>
