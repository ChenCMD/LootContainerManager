  ...
  def createInstr[F[_]: Async](transactor: Transactor[F]): LootAssetPersistenceInstr[F] = {
    ...
    new LootAssetPersistenceInstr[F] {
      ...
      override def upsertLootAsset(lootAsset: LootAsset): F[Unit] = {
        createUpsertLootAssetTransaction(lootAsset).transact(transactor)
      }
      ...
      override def deleteLootAsset(asset: LootAsset): F[Unit] = for {
        id <- asset.id.fold(SystemException.raise("Asset ID is not defined"))(_.pure[F])
        id <- Async[F].pure(NonEmptyList.one(id))
        program = for {
          _ <- makeItemsDeleteQuery(id).update.run
          _ <- makeAssetsDeleteQuery(id).update.run
        } yield ()
        _ <- program.transact(transactor)
      } yield ()
